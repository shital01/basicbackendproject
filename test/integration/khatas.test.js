...
//Coverage for cover and path not taken good idea
	describe('POST/',()=>{
		...
		it('should return 400 if validation khata failed due to friendName',async()=>{
			...
			expect(res.body.unsavedEntries[0].error).toBe('"friendName" must be a string');
		...
		it('should return 400 if validation khata failed due to rotationPeriod',async()=>{
			...
			expect(res.body.savedEntries.length).toBe(1);
			expect(res.body.unsavedEntries.length).toBe(1);
			expect(res.body.unsavedEntries[0].error).toContain('rotationPeriod');		
		});
		//Path-07
    ...
    expect(Object.keys(transactions[0])).toEqual(
				expect.arrayContaining(['settledFlag','_id','friendName','friendPhoneNumber','userId','userName','localId','userPhoneNumber','updatedTimeStamp', "interestRate", "interestType", "rotationPeriod"]))				
...
/*************PUT*********/
	...
	describe('PUT/',()=>{
		...
		//Path-03
		it('should return 400 if validation khata failed due to settledFlag',async()=>{
			settledFlag = null;//or false array input
			payload={settledFlag,khataId};
			const res = await exec();
			expect(res.status).toBe(400);
		})
		//Path-04
		it('should return 400 if validation khata failed due to friendName',async()=>{
			friendName = 123;
			payload = {friendName,khataId}
			const res = await exec();
			expect(res.status).toBe(400);
		})
		//Path-05
		it('should return 400 if validation khata failed due to friendPhoneNumber',async()=>{
			friendPhoneNumber = 123;
			payload = {friendPhoneNumber,khataId}
			const res = await exec();
			expect(res.status).toBe(400);
		})
		//Path-06
		...
		it('should return 400 if validation khata failed due to rotationPeriod',async()=>{
			rotationPeriod = 123;
			payload = {rotationPeriod,khataId}
			const res = await exec();
			expect(res.status).toBe(400);
		})
		//Add cAse if local Id is also added in validation function
		//Path-08
		it('should return 400 if validation khata failed due to invalid ID',async()=>{
			khataId="1";
			payload ={khataId}
			const res = await exec();
			expect(res.status).toBe(400);
		...
		//Path-09
		it('should return 400  if khata not found with given ID/if Already delete ',async()=>{
			khataId=userid;
			payload ={khataId}
			const res = await exec();
			expect(res.status).toBe(400);
		...
		*/
		...
		it('should update the khata and return valid response if valid khata ',async()=>{
			payload={settledFlag:true,khataId}
			const res = await exec(payload);
			expect(res.status).toBe(200);
