	...
	try {
		...
		allTransactions.forEach((transaction) => {
			...
			const timeElapsed = Math.floor(
				(Math.floor(Date.now()) - giveDate) / (24 * 60 * 60 * 1000),
			); //Note-02 look for possible optimizations
			...
			const CompountInterest =
				...
				Math.pow(
					1 +
						(interestRate * rotationPeriod) /
							(100 * interestPeriod),
					n,
				);
			const fractionInterest =
				(interestRate * d) / (interestPeriod * 100);
			const finalAmountForThisTransaction =
				CompountInterest * (1 + fractionInterest);
			console.log(
				principalAmount,
				interestRate,
