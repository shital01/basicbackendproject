...
function calculaterhelper(amount,testDate,interestRate,interestPeriod,rotationPeriod,testdate,olddata,olddates){
  ...
  while(flag){
    ...
    if(startDate>todaydate){
      lastdate=startDate;
      startDate= startDate.setMonth(startDate.getMonth() - (rotationPeriod/30));
...
async function calculateTotalAmount() {
  try {
    ...
    allTransactions.forEach(async(transaction) => {
      ...
      const n = calculaterhelper(startamount,inputDate,interestRate,interestPeriod,rotationPeriod,testdate,olddata,olddates);//return n,d
