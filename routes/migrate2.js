...
async function createKhataTable(data) {
    ...
    try {
        ...
        for (const transaction of data) {
            ...
            const pairKey = [senderPhoneNumber, receiverPhoneNumber].sort().join(',');
            ...
            const senderUser = await db.collection('User').findOne({ PhoneNumber: senderPhoneNumber });
            const senderUserId = senderUser ? senderUser.UserId : null;

            // Find userId based on receiverPhoneNumber
            const receiverUser = await db.collection('User').findOne({ PhoneNumber: receiverPhoneNumber });
